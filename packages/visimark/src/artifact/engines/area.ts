...
const LABEL_H = 26;
const POINT_R = 2.5;
/** fixed, not a style option: without some transparency, overlapping series
 *  hide each other entirely, the same necessity that gives `bar` its legend */
const FILL_OPACITY = 0.55;

/**
 * The area engine: each series fills from its line down to the zero
 * baseline, in its own grey at a fixed opacity so overlapping series stay
 * legible, with a full-opacity stroke on top to keep edges crisp.
 */
export function area(input: EngineInput): EngineResult {
  const { series, labels } = input;
  if (series.length === 0) return { err: "an area chart needs a series" };
  ...
  series.forEach((s, si) => {
    const pts = s.values.map((v, r) => `${n2(x(r))},${n2(y(v.toNumber()))}`);
    const path =
      `M ${n2(x(0))} ${n2(baseline)} L ${pts.join(" L ")} ` +
      `L ${n2(x(rows - 1))} ${n2(baseline)} Z`;
    body.push(`<path d="${path}" fill="${fills[si]!}" fill-opacity="${FILL_OPACITY}"/>`);
    body.push(
      `<polyline points="${pts.join(" ")}" fill="none" stroke="${fills[si]!}" ` +

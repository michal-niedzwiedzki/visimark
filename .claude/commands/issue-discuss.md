---
description: Summarize the session's review discussion on a GitHub issue and, on approval, post it as an issue comment
argument-hint: <issue-number>
allowed-tools: Bash(gh:*), Bash(mktemp:*), Bash(cat:*), Read, AskUserQuestion
---

You are running the **discussion** stage of the issue-review workflow — it
sits between `/issue-review` and `/issue-decide`. The design has the maintainer
"talk it over with the agent in the session" (design [§1](../../docs/visimark-design.md#1-purpose)); that back-and-forth is
...
Refuse (print the reason, do nothing else) if the issue is already decided —
...
"Discussion recorded. Run `/issue-decide <n>` when ready."

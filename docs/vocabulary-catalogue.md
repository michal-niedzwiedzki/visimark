...
| Presentation-only `%` display sigil on prose anchors | Optional trailing `%` on a scalar prose anchor (`<!--vmark=sheet.scalar%-->`). `fmt` writes stored × 100 at precision − 2 with a trailing `%`, and a leading minus when the ratio is negative. `check`'s verdict stays numeric; a `STALE` line prints both sides in the visible percent form; `PRECISION` when the binding's width is below 2. Two anchors of one scalar may disagree (bare vs `%`); `%` mixed with a unit in the same span is `UNIT`; a date, string, or chart/image target is `TYPE`. Scientific `^` is out of scope ([#142](https://github.com/michal-niedzwiedzki/visimark/issues/142)). Amends [§3](visimark-design.md#3-document-model), [§9](visimark-design.md#9-write-back), [§10](visimark-design.md#10-error-taxonomy); one closed carve-out of [§14](visimark-design.md#14-deferred). | `check` already accepts `N%` as the stored ratio; `fmt` currently rewrites it to `toFixed` on the first value change. One stored scalar, percent inside the strong node, no second binding and no arithmetic. Deal-desk / executable-documentation prose wants that shape, which a trailing `%` in human-owned text (the `PLN` pattern) cannot do, because `%` is not a unit ([§7](visimark-design.md#7-numeric-semantics)). Document-local. Explicitly not [#41](https://github.com/michal-niedzwiedzki/visimark/issues/41). Cost is **S**. | Carves out of [§14](visimark-design.md#14-deferred)'s deferred output formats and sits next to [§1](visimark-design.md#1-purpose)'s presentation non-goal. `%` already means postfix scaling in the block language. `precision − 2` rewrites in-tree `20.00%` / `50.00%` to `20%` / `50%` once the sigil is added. Inert: a `%` sigil on a currency total is legal. | [#140](https://github.com/michal-niedzwiedzki/visimark/issues/140) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/140#issuecomment-5775547789) |
...
| Prose notation for unary vocabulary (`\|x\|`, `⌊x⌋`, `⌈x⌉`, `√(x)`) | language feature | [#64](https://github.com/michal-niedzwiedzki/visimark/issues/64) | [#136](https://github.com/michal-niedzwiedzki/visimark/pull/136) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/64#issuecomment-5759705769) |

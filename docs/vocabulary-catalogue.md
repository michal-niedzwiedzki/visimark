...
| Publish `.pre-commit-hooks.yaml` | Adds a `visimark` hook id consumers reference via `repo: https://github.com/michal-niedzwiedzki/visimark`, `rev: <tag>`, so the `pre-commit` framework's whole ecosystem gets `check` with a four-line config instead of the hand-copied `repo: local` recipe in [`ci.md`](ci.md) chapter 23. `entry:` dispatches to whichever of an already-on-`PATH` `visimark`, `bunx`, or `npx` it finds, so a Bun-only machine — which has no `npx` at all — is covered, not merely deprioritised, and the PATH-first branch dogfoods this repo's own branch build the way `action.yml` already does. | Cheapest distribution surface in the roadmap's section 2 — no plugin architecture involved, reuses the unmodified `check` exit-code contract, and follows this repo's own established idiom for resolving Node vs Bun (`action.yml`'s fallback branch, the `bin/visimark` launcher from #29). | The pinned version baked into `entry:`, needed to keep the pin coherent with a consumer's `rev:` (the same reasoning `action.yml`'s own `version` input already uses), makes `.pre-commit-hooks.yaml` a fifth file `ci.yml`'s version-agreement check and `docs/releasing.md`'s release checklist must cover. | [#149](https://github.com/michal-niedzwiedzki/visimark/issues/149) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/149#issuecomment-5780522480) |
---

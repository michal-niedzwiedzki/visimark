...
| Record the browser bundle's size as a checked ceiling | Adds an assertion (alongside the `node:`-builtin checks in `test/playground/browser-graph.test.ts`) that fails CI when `docs/vendor/visimark-browser.js` exceeds a recorded byte ceiling — the current 299,700-byte baseline plus a margin fixed in the spec — so bundle growth is a reviewable, citable CI failure rather than an unwatched minified-file diff. | Closes the second of three [#176](https://github.com/michal-niedzwiedzki/visimark/issues/176) spike-check follow-ups at minimal cost; reuses the file that already states the "what may enter this bundle" policy rather than adding a new one; a ceiling-with-margin avoids the byte-exact-pin churn `playground-bundle` was pinned to Bun 1.4.2 specifically to avoid; touches no published surface. | The margin is unset and must be chosen and justified in the spec; a real engine change that grows the bundle will require a one-line ceiling bump, ongoing (intended) maintenance; the shared test file's header comment currently narrates a single invariant and needs to distinguish two once this lands; the issue's own doc-invalidation list misnamed `docs/ci.md`, which does not need to change. | [#191](https://github.com/michal-niedzwiedzki/visimark/issues/191) |  |
---

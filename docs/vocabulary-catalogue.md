...
| Prose notation for unary vocabulary (`\|x\|`, `⌊x⌋`, `⌈x⌉`, `√(x)`) | Four prose spellings for shipped mappers — `\|x\|`→`ABS(x)`, `⌊x⌋`→`FLOOR(x, 1)`, `⌈x⌉`→`CEILING(x, 1)`, `√(x)`→`SQRT(x)`. The delimiter pairs are one table-driven case in the parser; `√` is the `Σ` lexer substitution. `\|x\|` nests, because position decides open versus close. Legal wherever the named call is; `fmt` never rewrites either spelling. | Expressively neutral: each form resolves to an existing `Call` before arity, evaluation, the dependency walk or `infer`, so no new semantics, finding code or precision rule ([§4](visimark-design.md#4-syntax), [§7](visimark-design.md#7-numeric-semantics)). `\|` cannot collide with table syntax — expressions live only in fenced blocks — so §4's stated reason for excluding it is stale. `⌊x⌋` and `⌈x⌉` are the mathematical whole-number floor and ceiling, so the step `1` is the notation's own meaning. A formula a numerate reader recognises on sight; the capstone's `assert ABS(variance) <= 0.05` is the motivating document. ~36 lines in the lexer and parser. | Not expressive: existing vocabulary computes everything these do, so it is judged on cost, as `Σ` ([#43](https://github.com/michal-niedzwiedzki/visimark/issues/43)) was, and is not precedent for `∏`, `∫` or any other glyph. `⌊`, `⌋` and `√` are not on a keyboard. Two spellings of one function is one more thing a reader learns. Writing the bars inside a Markdown table needs an escaped bar. Fixes the LSP hover range, which assumed a call's name is `name.length` wide and already misplaced `Σ(Net)`. Dropped from the original bundle: `fmt --convert-notation`, `infer --notation` (`infer --math` is a section F issue), the §1 audience amendment and a standing criterion-4 exemption. | [#64](https://github.com/michal-niedzwiedzki/visimark/issues/64) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/64#issuecomment-5759705769) |

...
| Refuse unrecognised and misplaced CLI options | Every command exits `2`, with a `visimark: …` line on stderr, for an option it does not know (`--jsonn`) or one that belongs to another command (`fmt --write`, `check --get`), before it reads or writes any file; under `--json` the refusal is a `USAGE` envelope. Replaces "unrecognised options are ignored" in `cli-reference.md` and `ci.md`, and generalises the `--scenario` refusal. | Applies constraint 3 to how the CLI is invoked: today several invocations run and exit `0`, or fail naming the wrong problem, while doing something other than what was asked, and a CI job or agent cannot tell. `ci.md` already carries a section and a troubleshooting row on the `--jsonn` trap. Small: a per-command option table, a leftover check, and the existing `closest` for the did-you-mean. | Breaking for any job that passes an option to the wrong command or to an engine that lacks it (for example Action `args: --fix-dates` with `command: check`), and it reverses a stated forward-compatibility promise. `runCli` is public, so embedders see it. Touches many docs and three tests that pin ignoring. Needs a changelog migration note (ships as 0.1.7), and moves the shipped `SCENARIO` code for a misplaced or valueless `--scenario` to `USAGE`. | [#121](https://github.com/michal-niedzwiedzki/visimark/issues/121) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/121#issuecomment-5758855875) |
---

...
| `--json` on all document CLI commands | Every document-oriented command (`check`, `fmt`, `infer`, `eval`, `explain`) accepts `--json` and emits a deterministic JSON report of that command's result. Default text output, evaluation, writes and exit codes stay unchanged. Public CLI is `--json` only — no `--format` in this issue. Today's flat `eval --json` map is **broken once** into a namespaced envelope (`values`, `assertions`, `charts`). | CI and scripts can consume findings by `code` and `class` without parsing aligned terminal text, which [cli-reference.md](cli-reference.md) does not treat as an API. Same product as [§11](visimark-design.md#11-cli)'s `eval --json`, applied consistently. Constraint 4 holds if the payload is command results plus the engine version — no clock, locale, or network. | Public schema is a new API surface to version. Breaking `eval --json` is cheaper now than two schemas; a binding named `assertions` would otherwise collide. Editor plugins already consume engine findings via the LSP, so that motivation is weaker than CI. JSON numbers for decimals would dent [§7](visimark-design.md#7-numeric-semantics); values stay decimal strings. Cost **M**: result models, formatters, tests, `cli-reference`. | [#60](https://github.com/michal-niedzwiedzki/visimark/issues/60) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/60#issuecomment-5609768148) |

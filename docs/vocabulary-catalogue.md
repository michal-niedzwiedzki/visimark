...
| Import the published package under Node and Bun in CI, not just run its `bin` | `smoke-node` and `smoke-bun` in `ci.yml` each gain a step that does `import("visimark")` (or the runtime equivalent) against the installed tarball and calls one real export (`check`, built via `locate`/`build`), in addition to their existing `bin`-only exercise. No change to any published command, option, exit code, or `--json` shape. | Closes a real gap: #170 shipped an `import("visimark")`-breaking manifest bug under Bun while every existing CI check (including the `package.test.ts` static `exports`-vs-`files` guard added in #179) passed. Cheapest of the three #176 follow-ups — reuses the two jobs that already isolate the runtime and install the packed tarball; no new job, artifact, or runner. | The sketch script in the issue calls `check()` with a raw string, which does not match its real `(model: DocModel, opts) => CheckResult` signature — the landed script must build the model via `locate`/`build` first. `smoke-bun`'s step must run under the job's existing Node-excluding `PATH` reset or it silently stops testing what it claims to. | [#190](https://github.com/michal-niedzwiedzki/visimark/issues/190) | `NEW` |
---

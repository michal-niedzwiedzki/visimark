...
| Enforce the Node support policy (current LTS + latest stable, both blocking) | CI Node pins move from numeric (`20`) to aliases (`lts/*`, `latest`); a new `node-support-policy` job asserts `engines.node`'s floor matches the live LTS major and that no workflow or `.nvmrc` names a numeric Node version; `engines.node` moves `>=18` → `>=24` in `visimark` and `visimark-mcp`; `acceptance-node`/`smoke-node` become an `["lts/*", "latest"]` matrix, `fail-fast: false`. | Closes 17 months of silent floor rot (floor claimed `>=18`, EOL a year prior, exercised by nothing); the tested pin was itself 4 months stale the day it was written; alias-based pins cannot go stale the same way; both guards were tested against injected regressions. | `node-support-policy` (and CI generally) goes red on every future LTS promotion, by design; matrix job names gain `(lts/*)`/`(latest)` suffixes, requiring branch-protection required-checks to be repointed; `action.yml`'s composite Action still hardcodes `node-version: 20` and sits outside the guard's scanned scope, reproducing the same defect class one file away. | [#184](https://github.com/michal-niedzwiedzki/visimark/issues/184) |  |
---

...
| `space` and `report` statements | Two statements in a `vmark` fence, beside `chart`. `space <param> step <step>` sweeps the domain a param already declares ([#241](https://github.com/michal-niedzwiedzki/visimark/issues/241)). `report <name> [options]` names a module shipped with the tool (`ledger`, `deltas`, `gates`, `best`, `forbidden`). `check` parses both and resolves names; it does not run the questions, so a document that passes `check` today still passes. `fmt` does not rewrite the lines. Touches block syntax, evaluation (a sheet with `report` is asked once per question when simulated), and the finding set (new `TYPE`s). No write-back and no artifact. | The questions and the readings sit in the fence with the formulas. The report set is closed, the same shape as chart engines, so constraint 4 holds. An inexact step is a `TYPE`, not a dropped endpoint. | Depends on #241; if that domain does not land, `space` has nothing to consume. The motivating `#runs` sheet has `report` lines and no `space`, and the issue does not say what the questions are. The five report names are listed without the rule each one computes. | [#258](https://github.com/michal-niedzwiedzki/visimark/issues/258) | |
...
|------|------|---------|--------|----------|----------|
| Declared domain on `param` | language feature | [#241](https://github.com/michal-niedzwiedzki/visimark/issues/241) | [#246](https://github.com/michal-niedzwiedzki/visimark/pull/246) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/241#issuecomment-5821299735) |

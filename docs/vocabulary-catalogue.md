...
| Import the published package under Node and Bun in CI, not just run its `bin` | `smoke-node` and `smoke-bun` in `ci.yml` each gain an assertion, appended to their existing global-install step, that resolves `visimark`'s real `node_modules` directory from its bin symlink and imports both `visimark` and `visimark-mcp` as libraries from inside it — `check(build(locate(...)))` and `engineVersion()` respectively — in addition to their existing `bin`-only exercise. No change to any published command, option, exit code, or `--json` shape. | Closes a real gap: #170 shipped an `import("visimark")`-breaking manifest bug under Bun while every existing CI check (including the `package.test.ts` static `exports`-vs-`files` guard added in #179) passed. Cheapest of the three #176 follow-ups — reuses the two jobs that already isolate the runtime and install the packed tarball; no new job, artifact, or runner. Fully reversible without a release. | The issue's own sketch script would have failed on every build, correctly-published or not — a bare `import("visimark")` does not resolve after a global install from any directory, verified directly; the landed script must `cd` into the resolved package directory first. `smoke-bun`'s existing MCP handshake step was found, by reproducing #170's exact broken-exports shape, to already give incidental coverage of this class — real but dependent on `visimark-mcp` staying unbundled, and it surfaces as an opaque server crash rather than a statement about `visimark`'s importability. | [#190](https://github.com/michal-niedzwiedzki/visimark/issues/190) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/190#issuecomment-5794137088) |
| Cross-host equivalence check over the worked-example corpus | A new standalone `cross-host` CI job (`scripts/cross-host-check.ts`) runs `check`, `eval`, and `explain --json` for each of the twelve `docs/example-*.md` files through two hosts — the real CLI subprocess and the committed browser bundle, loaded via `node:vm` and re-enveloped by the same `report/json.ts` / `report/explain.ts` functions the CLI itself uses — and fails on any structural divergence, blocking from day one. No published command, option, exit code, stream, or `--json` shape changes. | Closes the one spike check from [#176](https://github.com/michal-niedzwiedzki/visimark/issues/176) not yet answered by work in the tree. Existing tests (`acceptance.test.ts`, `playground.test.ts`, `browser-graph.test.ts`) each check one host against fixed expectations; none diffs host against host, so the exact class of bug that took the playground down for 74 minutes (PR #93 → #95, fixed in #99) was invisible to CI. Small, reversible without a release, and the smallest useful version (two hosts, not three; structural not byte comparison) is the one specified. | The browser bundle has no code path producing a `--json`-shaped envelope on its own — `report/json.ts` pulls in `node:module` and cannot enter the browser graph, so the harness builds a comparable envelope from the bundle's raw `VM.check()` result on the Node side, which the spec pins down explicitly rather than leaving to the implementer. The two file-reading documents compare on a scoped envelope, excluding the one entry that legitimately differs because no reader was supplied. | [#189](https://github.com/michal-niedzwiedzki/visimark/issues/189) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/189#issuecomment-5793796093) |
---

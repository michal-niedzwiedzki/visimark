...
| Cross-host equivalence check over the worked-example corpus | A new CI job runs `check --json` for each of the twelve `docs/example-*.md` files through two hosts — the CLI (Node) and the committed browser bundle (loaded via `node:vm`, as `playground.test.ts` already does) — and fails on any divergence. No published command, option, exit code, stream, or `--json` shape changes. | Closes the one spike check from [#176](https://github.com/michal-niedzwiedzki/visimark/issues/176) not yet answered by work in the tree. Existing tests (`acceptance.test.ts`, `playground.test.ts`, `browser-graph.test.ts`) each check one host against fixed expectations; none diffs host against host, so the exact class of bug that took the playground down for 74 minutes (PR #93 → #95, fixed in #99) is invisible until a user reports it. Small, reversible, and the smallest useful version (two hosts, not three) is already the one proposed. | The browser bundle has no code path producing a `--json`-shaped envelope today — `report/json.ts` pulls in `node:module` and cannot enter the browser graph, so the harness must build a comparable envelope from the bundle's raw `VM.check()` result on the Node side. Byte-identical comparison needs the embedded `visimark` version field, key order, and number formatting pinned or normalized, none of which is stated as a contract today. Two of the twelve documents can only be compared under `skipped`. | [#189](https://github.com/michal-niedzwiedzki/visimark/issues/189) |  |
---

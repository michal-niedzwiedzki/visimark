...
| Algebraic types (units and denominations) | Replaces the bare numeric value with a **typed value**: every value carries a dimension, and the dimension computes. `+`/`-` demand compatible dimensions, `*`/`/`/`^` compose them algebraically (`kg * m` → `kg·m`, `USD/node/month * node` → `USD/month`), and a mismatch is a hard error rather than a plausible number. Denominations are declared on a table header (`Weight [kg]`), on a binding head (`Speed [m/s] = …`) and on operands mid-expression, each checked against the type the expression infers. Adds `conversion 1 kg = 1000 g` — a fourth statement kind beside bindings, `assert` and `chart` — dimension-based pruning in `infer`, types in `eval --json` output, typed series into charts, and at least one finding code. Explicitly **replaces** [§7](visimark-design.md#7-numeric-semantics)'s inert cell decoration and reverses [§15](visimark-design.md#15-known-tensions)'s "a unit is inert" compromise. | Turns a class of wrong formula from a plausible number into a hard error — [§1](visimark-design.md#1-purpose)'s auditability argument applied to meaning rather than arithmetic. A real motivating case exists **in this repo**: `example-invoice.md`'s `eur_total = gross_total / fx_eur` inverted is a rate-direction error that `check` passes green today, and the algebra rejects it structurally from one annotation. Constraint 4 holds fully and deliberately — conversions are written in the document, FX rates sit in the diff, and `conversion` selects no code, so the no-plugin rule is untouched. Fits the shape system unchanged: dimensions ride on values, so map/reduce and no-boolean-in-a-cell are unaffected. Renders unmodified everywhere; splicer and one-line-diff guarantee untouched. One annotation on an input protects every formula downstream of it. The v1 refusals — no SI-prefix inference, no multi-hop routing, no live FX, no affine units in multiplicative syntax — are the right ones. | [§14](visimark-design.md#14-deferred) defers this by name ("anything resembling dimensional analysis") and calls the flat v1 rule deliberate; [§15](visimark-design.md#15-known-tensions)'s inertness compromise is reversed rather than extended. Shipped `assert` ([#27](https://github.com/michal-niedzwiedzki/visimark/issues/27)) already catches the motivating FX case in one document line at zero implementation cost, so the claim over it is coverage-per-annotation, not capability. Unannotated documents are wholly dimensionless and catch nothing, so "one value model" is two-kinded at the use site. `[…]` in a header is a breaking change to [§6](visimark-design.md#6-name-resolution-and-scoping) — `Distance [m]` is a legal column name today and `explain` reports it as an input — and what replaces the [§7](visimark-design.md#7-numeric-semantics) cell decoration in existing `$`-decorated documents is unstated. Conversion performs arithmetic inside a binding printed nowhere, denting [§7](visimark-design.md#7-numeric-semantics)'s calculator promise; compound units have no canonical printed form; the ambiguity rules (`m` vs `min`) and the built-in conversion set were dropped from the update. Cost is **L–XL** — a dimension on `Value` reaches the lexer, parser, every operator, all eleven builtins, the graph, `check`, the whole `infer` pipeline, write-back and the report layer, breaks the flat `eval --json` contract, restales every committed chart SVG if units reach the output, and largely replaces `eval/units.ts`. | [#41](https://github.com/michal-niedzwiedzki/visimark/issues/41) |  |
...
| Declared local data imports with integrity stamps | language feature | [#66](https://github.com/michal-niedzwiedzki/visimark/issues/66) | [#68](https://github.com/michal-niedzwiedzki/visimark/pull/68) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/66#issuecomment-5626597963) |
| `unlabelled` clause for headerless CSV imports | language feature | [#70](https://github.com/michal-niedzwiedzki/visimark/issues/70) | [#72](https://github.com/michal-niedzwiedzki/visimark/pull/72) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/70#issuecomment-5637476576) |
| Human-readable column references and aliases (quoted headers, `is`) | language feature | [#86](https://github.com/michal-niedzwiedzki/visimark/issues/86) | [#88](https://github.com/michal-niedzwiedzki/visimark/pull/88) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/86#issuecomment-5668665880) |

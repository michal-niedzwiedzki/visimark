...
| `Σ` alias for `SUM` | Adds Greek capital Sigma (`Σ`) as an alternate lexical spelling of the `SUM` reducer; no new semantics — evaluates, type-checks, and infers identically to `SUM(col)`. | A recognisable nod to conventional summation notation; zero evaluation-model cost — identical reduce shape and arity to `SUM`. | A second spelling for one primitive is the pattern this project has twice rejected for operators (`%` infix, `//` division) as needless did-you-mean clutter; no motivating document — the issue frames the purpose as "mostly cultural," and `SUM(Net)` already expresses everything `Σ(Net)` would. | [#43](https://github.com/michal-niedzwiedzki/visimark/issues/43) |  |

...
| `CEILING(number, significance)` | Round `number` up to the nearest multiple of `significance`, toward positive infinity (`CEILING(17, 5) → 20`, `CEILING(-12, 5) → -10`). `significance` mandatory. | Complement of shipped `FLOOR`; same shape and error model; 2-arg form covers integer ceiling as `CEILING(x, 1)` without deciding overloading. | No document in the repo calls `CEILING` (criterion 4); algebraically `-FLOOR(-x, s)` already expresses it. | [#54](https://github.com/michal-niedzwiedzki/visimark/issues/54) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/54#issuecomment-5600795875) |

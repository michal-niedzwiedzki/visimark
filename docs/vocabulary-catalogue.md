...
| Record the browser bundle's size as a checked ceiling | Adds an assertion (alongside the `node:`-builtin checks in `test/playground/browser-graph.test.ts`, in its own comment block) that fails CI when `docs/vendor/visimark-browser.js` exceeds `BASELINE_BYTES * 1.1` — the current 299,700-byte baseline plus a 10% margin (329,670 bytes) — so bundle growth is a reviewable, citable CI failure rather than an unwatched minified-file diff. | Closes the second of three [#176](https://github.com/michal-niedzwiedzki/visimark/issues/176) spike-check follow-ups at minimal cost; reuses the file that already states the "what may enter this bundle" policy rather than adding a new one; a ceiling-with-margin avoids the byte-exact-pin churn `playground-bundle` was pinned to Bun 1.4.2 specifically to avoid; touches no published surface; the margin is expressed as a formula against the baseline, so a future bump only touches one constant. | A real engine change that grows the bundle past the margin requires a one-line ceiling bump, recorded as an in-file comment above the constant — ongoing, intended maintenance. The issue's own doc-invalidation list misnamed `docs/ci.md`; the spec drops it, since that file does not enumerate this repository's own CI jobs. | [#191](https://github.com/michal-niedzwiedzki/visimark/issues/191) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/191#issuecomment-5796346439) |
---

...
| Human-readable column references and aliases (quoted headers, `is`) | A quoted string is now legal as a column-rule left-hand side (`"Header" = expr`), matched byte-for-byte against an existing header only — it can never invent one. `"Header" is symbol` gives that column a short formula-facing name that behaves exactly like a native identifier column everywhere after it is declared. Widens `DUP` to also catch two header cells sharing identical text, closing a pre-existing silent collision for ordinary identifier headers too. | Printed tables stay untouched — no rewriting someone else's headers, which matters most for a table the author doesn't own at all. Write-back still owns only computed cells (constraint 2): neither form can create a header. Matching is the same raw-text comparison identifier headers already use — no normalisation, no guessing (constraint 3). `is` needs no new dependency-graph mechanism, just an earlier name-resolution pass, so the two-keyword cost the original proposal carried is gone. | One reserved word (`is`); a header containing a literal `"` remains unreachable by either form, since the string-literal grammar has no escape; `infer` needs a new proposal kind (a stopword-aware acronym heuristic) to make the feature actually useful on unfamiliar tables, which is a real but bounded addition. | [#86](https://github.com/michal-niedzwiedzki/visimark/issues/86) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/86#issuecomment-5668665880) |

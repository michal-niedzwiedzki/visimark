...
| Presentation-only display sigils on prose anchors (`%`, `^`) | Optional trailing `%` or `^` on a scalar prose anchor (`<!--vmark=sheet.scalar%-->`, `<!--vmark=sheet.scalar^-->`). `%` writes stored × 100 at precision − 2 with a trailing `%`; `^` writes scientific notation (`1.006e+9`). `fmt` applies the transform in the span it already owns; `check` `STALE`-compares against that rendering. Scalar prose anchors only. Amends [§3](visimark-design.md#3-document-model), [§9](visimark-design.md#9-write-back), [§10](visimark-design.md#10-error-taxonomy); carves two closed cases out of [§14](visimark-design.md#14-deferred). | `check` already accepts unsigned `N%` as the stored ratio; `fmt` currently rewrites it to `toFixed` on the first `STALE`. One stored scalar, two lossless renderings, no second binding and no arithmetic. Deal-desk / executable-documentation prose wants the percent inside the strong node, which a trailing `%` in human-owned text (the `PLN` pattern) cannot do, because `%` is not a unit ([§7](visimark-design.md#7-numeric-semantics)). Document-local. Explicitly not [#41](https://github.com/michal-niedzwiedzki/visimark/issues/41). Cost is **S**. | Carves out of [§14](visimark-design.md#14-deferred)'s deferred output formats and sits next to [§1](visimark-design.md#1-purpose)'s presentation non-goal. `%` and `^` already mean something in the block language. `precision − 2` disagrees with in-tree `20.00%` / `50.00%`, and is undefined below precision 2. `^` has no motivating document and no digit rule — the issue's own `1.006e+9` / `1006000000.00` pair does not share a width. Chart/image anchors share `sheet.name`. Inert: a `%` sigil on a currency total is legal. | [#140](https://github.com/michal-niedzwiedzki/visimark/issues/140) |  |

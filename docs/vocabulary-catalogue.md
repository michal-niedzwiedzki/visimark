...
| Human-readable column aliases (`is` / `now`) | Separates GFM header text from formula identifiers: `"Header" is symbol` aliases an existing input column; `"Header" now symbol = expr` is a column rule that writes to that header. The table is not rewritten. | Printed tables can keep spaces, units, and punctuation in headers while formulas use short identifiers. Ordinary Markdown (constraint 1). Write-back still owns only computed cells (constraint 2) if `now` cannot invent headers. | Two new reserved words; `now` reads as a clock (constraint 3, same family as rejected `TODAY()`). Matching, duplicate headers, scope, and whether `now` may create a column are unspecified. The issue treats automatic identifier normalisation as an existing default — it is not: today a header is a formula name only when it is already an identifier ([§3](visimark-design.md#3-document-model), [§4](visimark-design.md#4-syntax)). Quoted identifiers as binding names would close the same reachability gap with no `is` / `now` split. | [#86](https://github.com/michal-niedzwiedzki/visimark/issues/86) |  |

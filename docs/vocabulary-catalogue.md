...
| `FLOOR(number, significance)` | Round `number` down to the nearest multiple of `significance`, toward negative infinity (`FLOOR(17, 5) → 15`, `FLOOR(-12, 5) → -15`). `significance` mandatory. | Addresses the mandatory-argument con already on record for the row above; genuine bucketing/pricing-tier need if a document arrives. | No document in the repo exercises the 2-arg form today; zero-`significance` and negative-direction behaviour need explicit spec language; overlaps the row above. | [#53](https://github.com/michal-niedzwiedzki/visimark/issues/53) |  |

...
| `unlabelled` clause for headerless CSV imports | Extends [§19](visimark-design.md#19-declared-local-data-imports)'s import grammar with `unlabelled <col>,...`, a positional-names counterpart to `labelled` for CSV files with no header row: declared-name count must exactly equal the file's column count, duplicate names are rejected, and no ignored-column placeholder is introduced. | Closes a real gap — a headerless CSV cannot be imported at all today, since row 1 is always consumed as a header. Keeps ambiguity a hard `check` error rather than a guess (no manufactured `column1`/`column2` names), reusing #66's shipped stamp/path-gate/resolution machinery with minimal new surface. | No concrete motivating document attached — the case made is general ("headerless CSVs are common"), not an actual table. Several open design questions (mutual exclusivity with `labelled`, the neither-clause default, exact placement in the resolution order, and whether a mismatch is `IMPORT`) need closing before a spec can be written. | [#70](https://github.com/michal-niedzwiedzki/visimark/issues/70) |  |

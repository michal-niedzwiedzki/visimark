...
| Algebraic types (units and denominations) | Gives every value a dimension beside its number and makes the dimension compute: `+`/`-` demand compatible dimensions, `*`/`/`/`^` compose them algebraically (`kg * m` → `kg·m`, `km / h` → `km/h`), and a mismatch is a hard error instead of a plausible number. Denominations are declared on a table header (`Weight [kg]`) and on a binding (`Speed [m/s] = …`), and the declared result type is checked against the inferred one. Adds `conversion 1 kg = 1000 g` — a fourth statement kind beside bindings, `assert` and `chart` — plus a conservative built-in conversion set, dimension-based pruning in `infer`, and at least one finding code. Replaces [§7](visimark-design.md#7-numeric-semantics)'s inert decoration and reverses [§15](visimark-design.md#15-known-tensions)'s "a unit is inert" compromise. | Turns a whole class of wrong formula from a plausible number into a hard error, which is [§1](visimark-design.md#1-purpose)'s auditability argument applied to meaning rather than arithmetic. Constraint 4 holds fully and deliberately — conversions are written in the document, currency rates are stated in the diff, and `conversion` selects no code, so the no-plugin rule is untouched. Fits the shape system unchanged: dimensions ride on values, so map/reduce and the no-boolean-in-a-cell rule are unaffected. Renders unmodified everywhere; the splicer and the one-line-diff guarantee are untouched. Prunes `infer` candidates on type rather than on another heuristic. The v1 scope refusals — no SI-prefix inference, no multi-hop routing, no live FX, no affine units in multiplicative syntax — are the right ones. | [§14](visimark-design.md#14-deferred) already defers this by name ("anything resembling dimensional analysis"), calling the flat v1 rule deliberate; [§15](visimark-design.md#15-known-tensions)'s inertness compromise would be reversed rather than extended. No real document motivates it — the issue's examples are constructed, and no worked example carries a calculation a dimension check would catch, so the acceptance suite has nothing to bite on. `[…]` in a header is a breaking change to header/name matching (a column genuinely named `Weight [kg]` is reinterpreted), and it is a second spelling alongside [§7](visimark-design.md#7-numeric-semantics)'s cell decoration with no stated precedence. Conversion performs arithmetic inside a binding that is printed nowhere, denting [§7](visimark-design.md#7-numeric-semantics)'s calculator promise. Compound units have no canonical printed form yet. Cost is **L–XL** — a dimension field on `Value` reaches the lexer, parser, every operator, all eleven builtins, the graph, `check`, the whole `infer` pipeline, write-back and the report layer, and largely replaces `eval/units.ts`. | [#41](https://github.com/michal-niedzwiedzki/visimark/issues/41) |  |

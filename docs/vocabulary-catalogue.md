...
| Declared local data imports with integrity stamps | A sheet may take its table from a declared local file instead of an inline GFM table: `vmark #benchmark from benchmark.csv delimited , labelled Id, Time at sha256:<digest>`. `at` locks the import to a full `sha256:` content digest — a Git commit reference is not supported; `fmt` writes and updates the digest, `check` verifies it and never updates it, and an unstamped import fails `check`. Imported sheets are read-only: no column rules, in any form. `§9` gains a fourth, read-side category — a **declared input** — distinct from the three the tool writes. | Large machine-generated datasets — benchmarks, measurements, exported query results — stop being unreadable 50,000-row Markdown tables while the computation and its interpretation stay in the document. The dependency and its exact identity sit adjacent in the text, so a reviewer sees both, and a changed input fails `check` until a human accepts it with `fmt`. Diffability improves rather than degrades: a one-row data change is one CSV line plus one digest span. [§18](visimark-design.md#18-generated-artifacts) already established a tool-written local file with a hard path gate this reuses verbatim, and no plugin point opens — data is imported, not code. Read-only imported sheets close the column-rule question outright rather than leaving it implied. | Amends constraint 4 to admit "the contents of explicitly declared local input files", and contests [§2](visimark-design.md#2-constraints-that-shaped-the-design)'s standing "values that genuinely come from elsewhere belong in an input column" — deliberately, on the declared-local-vs-ambient distinction. CSV parsing and type conversion reuse the existing table-cell literal grammar, but still add a full RFC 4180 parser, a new `IMPORT` finding, and a widened `STALE`/`SHEET`/`TYPE`. Cost **L**, and 600,000 cells is the first real test of [§8](visimark-design.md#8-evaluation)'s single-digit-millisecond reparse claim, since v1 re-hashes the file on every run. | [#66](https://github.com/michal-niedzwiedzki/visimark/issues/66) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/66#issuecomment-5626597963) |

...
| `unlabelled` clause for headerless CSV imports | Extends [§19](visimark-design.md#19-declared-local-data-imports)'s import grammar with `unlabelled <col>,...`, a positional-names counterpart to `labelled` for CSV files with no header row: declared-name count must exactly equal every row's field count, duplicate names are rejected, and no ignored-column placeholder is introduced. | Closes a real gap — a headerless CSV cannot be imported at all today, since row 1 is always consumed as a header. Keeps ambiguity a hard `check` error rather than a guess (no manufactured `column1`/`column2` names), reusing #66's shipped stamp/path-gate/resolution machinery with minimal new surface. `labelled`/`unlabelled` are mutually exclusive (`TYPE` error if both appear); omitting both keeps today's behaviour unchanged; per-row arity validation is new for `unlabelled` only, not retrofitted onto `labelled`. | The pre-review's concrete-document gap was closed by specifying a committed headerless fixture pair (`benchmark-headerless.{csv,md}`) rather than by pasting one into the issue itself. | [#70](https://github.com/michal-niedzwiedzki/visimark/issues/70) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/70#issuecomment-5637476576) |

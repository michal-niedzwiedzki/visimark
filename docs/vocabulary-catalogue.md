...
| Division by zero is `TYPE` | A zero divisor in `/` and `MOD` is a `TYPE` error (`division by zero`); a non-finite `Decimal` never becomes a `Value` (`result is not a finite decimal`). `check` reports `TYPE`, not `STALE`; `fmt` writes nothing for those bindings. | Restores [§7](visimark-design.md#7-numeric-semantics) (arithmetic is decimal; a reader re-adds it on a calculator) and constraint 3 ([§2](visimark-design.md#2-constraints-that-shaped-the-design): refuse rather than pick). Same reporting shape as `SQRT` of a negative. A scenario ([#119](https://github.com/michal-niedzwiedzki/visimark/issues/119)) can no longer make `eval` report `Infinity` as a value. Cost **S**. | `eval --json` `status` still tracks assertions only, so a document whose only problem is this `TYPE` remains `"ok"` with the failed binding omitted from `values` — existing contract, not a new hole. `0 ^ -1` and `(-2) ^ 0.5` share a generic message rather than a per-operator table. | [#122](https://github.com/michal-niedzwiedzki/visimark/issues/122) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/122#issuecomment-5746529695) |
...
| Scenario parameters (`param`, `eval --scenario`) | language feature | [#119](https://github.com/michal-niedzwiedzki/visimark/issues/119) | [#123](https://github.com/michal-niedzwiedzki/visimark/pull/123) | | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/119#issuecomment-5740581923) |

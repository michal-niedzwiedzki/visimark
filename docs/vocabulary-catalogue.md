...
| Enforce the Node support policy (current LTS + latest stable, both blocking) | CI Node pins move from numeric (`20`) to aliases (`lts/*`, `latest`); a `node-support-policy` job asserts `engines.node`'s floor matches the live LTS major and that no workflow, `action.yml`, or `.nvmrc` names a numeric Node version; `engines.node` moved `>=18` → `>=24` in `visimark` and `visimark-mcp`; `acceptance-node`/`smoke-node` are an `["lts/*", "latest"]` matrix, `fail-fast: false`. | Closes 17 months of silent floor rot (floor claimed `>=18`, EOL a year prior, exercised by nothing); the tested pin was itself 4 months stale the day it was written; alias-based pins cannot go stale the same way; both guards were tested against injected regressions. | `node-support-policy` (and CI generally) goes red on every future LTS promotion, by design; matrix job names gained `(lts/*)`/`(latest)` suffixes (`master` has no branch protection configured, so nothing needed repointing); `action.yml` carried the same stale `node-version: 20` pin, outside the guard's original scanned scope — closed as a small follow-up alongside a `CONTRIBUTING.md` fence defect found during decision. | [#184](https://github.com/michal-niedzwiedzki/visimark/issues/184) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/184#issuecomment-5790907466) |
---

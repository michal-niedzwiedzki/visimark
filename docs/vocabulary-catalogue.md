...
|---------|-----------------|------|------|---------|--------|
| Charts (`chart` statement, SVG artifacts) | A `chart #id as pie\|bar of Series[, …] labelled Col` statement in a `vmark` block; `fmt` generates a deterministic SVG under `charts/`, `check` reports it stale or missing via an input hash in the SVG metadata. Adds a third category of tool-owned output — files outside the document. | Charts stop being a second source of truth: one dataset, regenerated and CI-verified like every other derived value. Determinism is well specified — no clock, network, fonts or config — so constraint 4 holds. Forbidding formulas in declarations keeps the calculation model in one place, mirroring the existing refusal of `SUM(Price * Qty)`. Machine-readable end to end; an agent never needs to view the image. | [§1](visimark-design.md#1-purpose) lists "no presentation layer" as a non-goal, and an SVG cannot be audited in a diff — the property [§1](visimark-design.md#1-purpose) exists to provide. [§9](visimark-design.md#9-write-back) says the tool owns only computed cells and anchored values; writing files is a third category needing an opt-in flag by the `--fix-dates` precedent. One input change rewrites a whole SVG, against [§13](visimark-design.md#13-testing)'s one-line-diff guarantee. `#` is already the comment character, so the proposed syntax cannot lex; an image is not a valid anchor target. Cost is L — a rendering engine plus file, hashing and staleness subsystems. | [#36](https://github.com/michal-niedzwiedzki/visimark/issues/36) |  |

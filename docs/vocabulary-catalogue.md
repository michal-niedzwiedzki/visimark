...
| Scenario parameters (`param`, `eval --scenario`) | A `param NAME precision N = default LITERAL` statement declares a numeric scalar that may be varied from outside; every command but `eval --scenario` treats it as its default. `param` and `default` are contextual keywords, as the `chart` statement's are, so nothing new is reserved. `eval --scenario FILE\|-` evaluates the document under a strict flat JSON object of string-valued param values (unknown or non-param keys, JSON numbers, values wider than the declared width, and a bare value for a percent param are errors), quotes the scenario separately in its output, runs assertions under it and reports whether each failure holds on the defaults, and writes nothing. `--scenario` on any other command is exit `2`. | What-if questions stop costing an edit, a `fmt` and a revert, and stop leaving a diff nobody meant to commit. The defaults are the document: `check`, `fmt` and stored values never see a scenario, so two checkouts still agree. String-only values keep every digit exact, and together with strict keys, reject-not-round widths and the percent rule they keep constraint 3. A failing assertion under a pessimistic scenario is visible in the exit code, and marked as scenario-only when the defaults pass. `explain` lists the knobs an agent may vary. No new shape, no new owned category, no upgrade break. The motivating document is [`example-agent-budget.md`](example-agent-budget.md). | Needs a one-sentence amendment to [§2](visimark-design.md#2-constraints-that-shaped-the-design) constraint 4, because a scenario is an input the document does not declare. The percent rule is a scenario-only check: input columns still mix `30%` and `0.30`. A `param` default may not be narrowed by its declared precision, unlike every other binding ([§7](visimark-design.md#7-numeric-semantics)). A failed assertion under a scenario costs a second evaluation on the defaults. Refusing `--scenario` elsewhere is an exception to "unrecognised options are ignored" until [#121](https://github.com/michal-niedzwiedzki/visimark/issues/121) makes it the rule. Cost **M**. | [#119](https://github.com/michal-niedzwiedzki/visimark/issues/119) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/119#issuecomment-5740581923) |

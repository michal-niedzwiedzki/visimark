...
| Extension changelog entry for every release | A written rule in [`releasing.md`](releasing.md) — every release gets a dated `## X.Y.Z - YYYY-MM-DD` entry in `editors/vscode/CHANGELOG.md`, even a one-line "No editor-visible changes" — and one step in `ci.yml`, beside "every version-carrying file must agree", that fails the build when that file, and the root `CHANGELOG.md`, has no dated heading for the manifest version. No CLI option, exit code, stream or `--json` shape moves; the only machine affected is `ci.yml`, which gains one failing condition. | Closes the gap v0.1.6 exposed: the file is a release input read by `vsce` for the Marketplace page, and nothing opened it. Follows the precedent of the version-agreement step — an unenforced checklist step gets skipped on the release where it matters. Smallest useful version is the CI step alone; it passes on today's tree and on every PR that does not bump the version, so nothing open breaks, and it is undone by deleting the step with no release. | Enforces presence, not accuracy: an entry that says "No editor-visible changes" when the extension did change still passes, so the judgement of what the language server surfaces stays manual. The heading match must be anchored (`^## 0\.1\.6( \|$)`), or `0.1.6` matches `0.1.60`. Widened to the root `CHANGELOG.md` (the GitHub Release body, with the same tag-ahead-of-changelog failure), so the step is two files, not one. `## Unreleased` is optional in the extension file and still ignored by the check. Documentation debt: `releasing.md` (step 4, "Preparing the changelog", "Rules that bite"), `CONTRIBUTING.md`'s "Every version-carrying file must agree" paragraph. Cost is **S**. | [#131](https://github.com/michal-niedzwiedzki/visimark/issues/131) | `NEW` |
---

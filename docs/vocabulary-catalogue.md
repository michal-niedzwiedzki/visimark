...
| `EOMONTH(d, months)` | Last day of the month `months` away from `d`. Returns a plain date. | "Net EOM" payment terms are common, and it needs no month type — it yields an ordinary date. | Calendar/leap math; nudges past §5's explicit deferral of a month type. | — | `DEFERRED` |

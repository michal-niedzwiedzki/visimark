...
| Whitespace-substitution sigils on string prose anchors (`&`, `_`) | Two optional trailing sigils on an anchor comment targeting a string-valued scalar, asking `fmt` to join the value's words with `&nbsp;` or `_` instead of a plain space when writing that span — extends the anchor-comment grammar `%` (#140) and the proposed `^` (#142) already use. | Names a real, reproduced gap: `check` reports 0 problems both before and after a string-anchored value changes under unchanged prose (`docs/tutorial.md` ch. 9's documented "one current limit"). Display-only, matching `%`'s shape exactly. A whole-span-wrap form of `&` would also close a separately-reproduced, already-live danger — `fmt` silently deleting the word before a bare numeric anchor. | No in-tree document uses either sigil: `fmt`/`check` do not write or verify a string anchor at all yet, so the string half has nothing to attach to until string write-back is proposed and decided on its own (§14). The core mechanism — internal-join vs whole-span-wrap — is an open fork that changes which problem the feature actually solves, and the two candidate sigils don't share a timeline (`_` is inert until string write-back ships; whole-span `&` is useful today independent of it). Confirmed collision risk with `&nbsp;&nbsp;` already used as a field separator in this repo's own worked examples. | [#146](https://github.com/michal-niedzwiedzki/visimark/issues/146) |  |
...
| Presentation-only `%` display sigil on prose anchors | language feature | [#140](https://github.com/michal-niedzwiedzki/visimark/issues/140) | [#144](https://github.com/michal-niedzwiedzki/visimark/pull/144) | — | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/140#issuecomment-5775547789) |

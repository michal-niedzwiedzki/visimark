...
| `CEILING(number, significance)` | Round `number` up to the nearest multiple of `significance`, toward positive infinity (`CEILING(17, 5) → 20`, `CEILING(-12, 5) → -10`). `significance` mandatory. | Complement of shipped `FLOOR`; the seed already called next-standard-size / price-tier a genuine need; same shape and error model as `FLOOR`. | No document in the repo exercises `CEILING` (criterion 4); `FLOOR`'s spec recorded that shipping `FLOOR` does not by itself unblock `CEILING`. | [#54](https://github.com/michal-niedzwiedzki/visimark/issues/54) |  |

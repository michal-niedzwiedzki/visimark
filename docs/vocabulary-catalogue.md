...
| `--json` on all document CLI commands | Every document-oriented command (`check`, `fmt`, `infer`, `eval`, `explain`) accepts `--json` and emits a deterministic JSON report of that command's result. Default text output, evaluation, writes and exit codes stay unchanged. An internal formatter split leaves room for a later `--format yaml`; YAML itself is out of this issue. Existing `eval --json` becomes the first member of the convention rather than a one-off. | CI and scripts can consume findings by `code` and `class` without parsing aligned terminal text, which [cli-reference.md](cli-reference.md) does not treat as an API. Same product as [§11](visimark-design.md#11-cli)'s `eval --json`, applied consistently. Constraint 4 holds if the payload is command results only — no clock, locale, or network. | Public schema is a new API surface to version. Today's `eval --json` is a flat map of binding names plus `assertions`/`charts` keys, so a binding named `assertions` would collide. Editor plugins already consume engine findings via the LSP, so that motivation is weaker than CI. JSON numbers for decimals would dent [§7](visimark-design.md#7-numeric-semantics); values must stay decimal strings. Cost **M**: result models, formatters, tests, `cli-reference`, and a compat decision on `eval --json`. | [#60](https://github.com/michal-niedzwiedzki/visimark/issues/60) |  |

...
The language ships **ten functions and a fixed operator set**
...
| `SQRT(x)` | Non-negative square root of a non-negative number; a negative operand is a `TYPE` error reported once against the binding. | Real, recurring engineering need (diagonal / brace lengths, RMS, distance) — now with a verified motivating document. Map-shaped, arity 1, same class as `ABS` / `MOD`. `Decimal.js` `.sqrt()` is correctly-rounded decimal, so no binary-float precision cost (§7), unlike `SIN` / `LN`. The explicit negative-domain `TYPE` error is less ambiguous than the `^ 0.5` workaround's silent `NaN`. Established name (Excel / Sheets). | `(x) ^ 0.5` already reaches the value — the request is a readability argument (a bare `0.5` exponent reads as ambiguous intent beside genuine `^2` exponents), not a capability gap. Irrational result is rounded at the binding like any other number (§7); the one definitional point — negative operand is `TYPE`, never complex, never a silent `ABS` — must be pinned in §4. Single use in the one motivating document; §14's bar is recurring need. | [#18](https://github.com/michal-niedzwiedzki/visimark/issues/18) |   |

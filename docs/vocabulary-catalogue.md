...
| `EOMONTH(d, months)` | Last day of the month `months` calendar months from `d`; the day component of `d` is discarded. Returns a plain date. | "Net EOM" payment terms are common and unreachable by `date ± number`; needs no month type — it consumes a date and yields one; the leap/calendar math is already required by `date − date`. | Calendar/leap math in the engine; one definitional choice to pin in §5 — a `months` offset that overflows the year rolls over (month 13 → January); `months` must be an integer or `TYPE`. | [#6](https://github.com/michal-niedzwiedzki/visimark/issues/6) |  |

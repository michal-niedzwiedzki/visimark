...
| Publish `.pre-commit-hooks.yaml` | Adds a `visimark` hook id consumers reference via `repo: https://github.com/michal-niedzwiedzki/visimark`, `rev: <tag>`, so the `pre-commit` framework's whole ecosystem gets `check` with a four-line config instead of the hand-copied `repo: local` recipe in [`ci.md`](ci.md) chapter 23. | Cheapest distribution surface in the roadmap's section 2 — no plugin architecture involved, reuses the unmodified `check` exit-code contract, and follows an idiom (`system` + pinned `npx`) this repo already uses in `action.yml`'s fallback branch. | If `entry:` bakes a specific npm version to keep the pin coherent with `rev` (recommended, the way `action.yml`'s own `version` input already is), `.pre-commit-hooks.yaml` joins the four files `ci.yml`'s version-agreement check already enforces, becoming a fifth place a release commit must bump. | [#149](https://github.com/michal-niedzwiedzki/visimark/issues/149) | `NEW` |
---

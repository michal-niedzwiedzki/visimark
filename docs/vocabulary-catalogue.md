...
| A `remark`/`unified` plugin wrapping `visimark check` | New package exposing a `unified` transformer that calls the existing public `analyze()` and reports findings as `VFile` messages, so any `remark`/`remark-lint` pipeline (Docusaurus, Astro, and the wider `unified` ecosystem) gets visimark findings inline with its other lint rules via one config line. | Reaches an audience that will never invoke the CLI directly or read the landing page — the cheapest kind of distribution this project has, per roadmap section 2's own framing. Reuses `analyze()`, `lineOf()`, `isProblem()` and `ERROR_CODES`, all already public and already tested; no new engine surface needed for the recommended (a) route. | Pays a full re-parse per document per host build (chosen deliberately over new engine surface). Introduces a new package that, if version-pinned, becomes a candidate for `ci.yml`'s version-agreement check. Package name and per-finding `ruleId`/`source` conventions are undecided. | [#152](https://github.com/michal-niedzwiedzki/visimark/issues/152) |  |
---

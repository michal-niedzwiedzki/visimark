...
| Bun/Node parity as an always-on rule | A standing agent rule, `.agents/rules/runtime-parity.md`, linked from the rules row in `AGENTS.md` and cited from both runtime paragraphs in `CONTRIBUTING.md`. Bun and Node are both first-class for a published `bin`: in-repo development stays Bun-first, Node stays a CI-tested target, the `bin` entry points at an `sh` launcher (the pattern in `packages/visimark/bin/visimark`) rather than a `#!/usr/bin/env node` file, and the install and run-without-installing lines for a published command are written in both forms (`npm i -g` / `bun add -g`, `npx` / `bunx`). A snippet that documents a named Node runner stays `npx`. No command, exit code, stream, or `--json` shape changes, and no CI job changes. | The policy is already enforced and already scattered: the #29 spec states the Bun direction, `CONTRIBUTING.md` and `acceptance-node` state the Node direction, and the launcher comment states the mechanism. None of those is loaded when an agent adds a package. #169 is the first new published `bin` since the launcher, and a stock MCP template ships the shebang that made `bun add -g visimark` fail. The file is short, reversible by deletion, and needs no release. [§1](visimark-design.md#1-purpose) holds because the published checker still has to start, and [§2](visimark-design.md#2-constraints-that-shaped-the-design) constraint 4 holds because a document's result does not depend on which runtime launched it. | An always-on rule spends context on every turn, so it stays at the length of the rule file. It does not rewrite `docs/ci.md` or the README CI one-liner. It adds no shebang grep: a grep of every file under `bin/` would fail `packages/visimark/bin/visimark.js`, which #29 kept and which `acceptance-node` executes with `node`. Packages with no `bin` are not given a second CI job. This does not reopen the launcher ([#29](https://github.com/michal-niedzwiedzki/visimark/issues/29#issuecomment-5572442105)); it generalises the expectation that spec applied to one file. | [#171](https://github.com/michal-niedzwiedzki/visimark/issues/171) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/171#issuecomment-5785197844) |
| `--no-artifacts` for `fmt` | A CLI surface change and nothing else. `fmt` writes two kinds of thing: it splices bytes into the document it was handed, and it writes whole SVG files at paths derived from that document, creating directories on the way. The flag declines the second and leaves the first alone — computed cells, anchored values and import stamps are still spliced, `--fix-dates` still applies. It is legal on `fmt` alone; anywhere else it is `visimark: --no-artifacts is only valid with fmt`, exit `2`. The `--json` envelope gains an always-present `artifactsSkipped` count, per-file and in the summary, and the human summary line names it (`unchanged, 5 artifacts skipped`). It does **not** touch `check`: a missing or stale artifact is still `STALE`, still counted, still exit `1`. Spec: [`design/a-no-artifacts-flag-for-fmt-spec.md`](design/a-no-artifacts-flag-for-fmt-spec.md). | Answers a real gap in the machine contract: `fmt` writing files the caller never named surprises any caller that assumed it edits the file it was handed — a read-only checkout, a sandboxed build, a container with a single file mounted, a CI job running `fmt` to diff the result rather than keep it. Additive, opt-in and reversible: the default path is byte-for-byte what it is today, so section F's breaking-change test passes cleanly — no CI job, script or composite-Action invocation behaves differently, and `action.yml` passes `args` through verbatim so the flag needs no Action change. Costs almost nothing to build: one row in the `args.ts` option table yields the misplacement refusal, the no-value hint and the did-you-mean with no new machinery. Exit codes do not move, and that is not even a decision — `fmt()` already filters `STALE` out of `unfixable` before the write loop runs, so a document with five missing charts reports `unfixable: []` today. Stays inside [§1](visimark-design.md#1-purpose) and does not touch [§2](visimark-design.md#2-constraints-that-shaped-the-design) constraint 4, because the flag is explicit on the command line rather than inferred from the environment. | The name is the risk. [`ci.md`](ci.md) §19 is explicit that charts are committed state and that `.gitignore`-ing them makes CI fail correctly, and `--no-artifacts` is exactly what someone will reach for to make that failure go away. The guard is wording, not code: §19 names the flag and says plainly it is not that, and the reference row and the skill guidance both carry the distinction — it declines a write, never a verdict. Silencing the finding as well was refused: it would be a way to get `0 problems` on a document whose committed chart contradicts its data, the class of failure the finding exists to prevent. It also leaves a behaviour that currently holds by accident load-bearing, so the spec pins the exit code with a regression test. Not a superset of a `--dry-run` on `fmt`, which would decline the value repairs too and is the opposite of the motivating case; that stays unfiled. Does not reopen charts ([#36](https://github.com/michal-niedzwiedzki/visimark/issues/36)) or wait on the MCP server ([#169](https://github.com/michal-niedzwiedzki/visimark/issues/169)), which merely exposed the gap. | [#168](https://github.com/michal-niedzwiedzki/visimark/issues/168) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/168#issuecomment-5785369119) |
---

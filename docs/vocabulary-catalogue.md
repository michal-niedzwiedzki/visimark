...
| Declared domain on `param` (`in [lo, hi]` / `in { … }`, optional `integer` preset) | A `param` header may add a domain clause narrowing the legal scenario values below its declared width. The default must lie in the domain (`TYPE`, exit `1`). An out-of-domain `eval --scenario` value is refused before evaluation (`SCENARIO`, exit `2`), distinct from an in-domain value that fails an `assert` (exit `1`, values printed). `eval` and `eval --scenario` gain a `params:`/`params` block naming each domain-bearing param's domain; `explain`'s existing `params:` block is unaffected pending an open fork. No `fmt` write-back. | Lets a document state the askable envelope for a scenario search inline, rather than in a script beside the file — closing a gap `docs/design/scenario-params-spec.md` §7 named as future work when `#119` shipped. Stays inside the shape system (still a scalar); adds no plugin surface; a document with no domain-bearing param is byte-for-byte unaffected by `check`, `fmt`, `infer` or plain `eval`, confirmed by running the branch CLI on the issue's own "before" snippet. | Several syntax choices genuinely open (bracket/keyword spelling, half-open intervals, `natural`/`positive` semantics, `step`) — a spec has real work to do before `/issue-decide` can close it. Whether membership widens `TYPE` or earns a new code is undecided. `explain` and `ref --json` printing the domain is unresolved, risking a temporary divergence between `eval`'s and `explain`'s param reporting. | [#241](https://github.com/michal-niedzwiedzki/visimark/issues/241) |  |

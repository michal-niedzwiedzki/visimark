...
| Pin what `markdownlint-rule-visimark`'s HTML-comment restoration actually restores, and decide what a failed `analyze()` reports | `source.ts`'s doc comment currently claims byte-for-byte restoration; it is not true for comments inside fenced/indented code or inline code spans, or for a CRLF document's multi-line comment — both cases already degrade safely but are untested and undocumented as such. Separately, each of the seventeen `markdownlint` rules calls `findingsFor()`/`analyze()` with no `try`/`catch`, so an engine exception on one document currently surfaces as seventeen copies of the same stack trace instead of one violation; the spec adds an eighteenth rule, `visimark-engine-error`, that reports it once. | Both halves are small and reversible; the package is `UNRELEASED` ([#153](https://github.com/michal-niedzwiedzki/visimark/issues/153)/[#164](https://github.com/michal-niedzwiedzki/visimark/pull/164)), so neither is ever a change in published behaviour. The doc-comment correction closes a real gap between two files (`source.ts` and `markdownlint-rule-spec.md` §2.2) that independently overstate the same guarantee. Reducing seventeen duplicate exceptions to one is a real ergonomics win for anyone running `markdownlint-cli2` against a document that breaks the engine, and the `analyze()` memo now caches the failure too, so a broken document still costs one `analyze()` call rather than seventeen. | Reusing one of the seventeen existing `FindingCode` rules to report the failure was ruled out during spec drafting: `markdownlint` prints a rule's fixed description before its detail, so any of them would glue on an unrelated, misleading description. A new named rule is one more line in the package's rule-count documentation (README, `docs/ci.md` ch. 25) that must move from seventeen to eighteen. No CLI, exit-code, or `--json` change — the win is scoped to this one package's error path. | [#173](https://github.com/michal-niedzwiedzki/visimark/issues/173) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/173#issuecomment-5786173572) |
| An MCP server exposing the engine to agents | A new published package, `packages/visimark-mcp`, serving all six commands over stdio as eight tools (`visimark_check`, `_eval`, `_explain`, `_infer`, `_fmt`, `_ref`, `_fmt_apply`, `_infer_apply`), plus resources (a generated variant of the authoring skill, `cli-reference.md`, `function-reference.md`, the worked invoice and its drift twin) and two prompts — take over an existing document, author a new one. Read tools take **either** `{ path }` **or** `{ content }`, so an agent can check a table it is drafting in context without writing a temp file; `content` mode surfaces the engine's existing `"skipped"` state for imports and charts rather than letting it read as a clean pass. `visimark_fmt` and `visimark_infer` only plan. Applying is a separate call, gated on **both** `--allow-write` and a host-declared root, and refused when the plan's `sha256` no longer matches the file. Imports the library as `remark-visimark` does — never shells out. Adds npm and MCP-registry publish legs to `release.yml` in the existing ask/publish-or-skip/gate shape. Spec: [`design/mcp-server-spec.md`](design/mcp-server-spec.md). | The contract work was largely done and was verified against the tree: `--json` is on all six commands (`cli-reference.md:63-68`), the three exit-code classes are live, `isProblem()` is exported, and `index.ts` already exposes the content-in API the playground and `remark-visimark` run on. Exit `1` becomes a successful tool call reporting problems — only the `2` class is an MCP error — and the `--json` envelope is consumed as the wire format rather than reshaped, so there is one consumed contract and not two that can drift. Additive and opt-in: no existing CI job, script, `action.yml` invocation or installed extension behaves differently. Reaches an audience the in-repo skill cannot — an agent working in someone else's repository — and the plan/apply split costs almost nothing, since `planFmt` and `planInfer` already exist. | Commits the project to a sixth versioned manifest in lockstep, a fourth registry in the release gate, and a public tool surface with no deprecation channel comparable to a CLI flag's. The engine's public surface grows by `writeArtifact`, `ArtifactWrite` and `resolveArtifactPath`: the issue's claim that the engine does no filesystem writing is wrong — `artifact/write.ts` holds the O_EXCL / O_NOFOLLOW / marker gate — and exporting it was judged better than having a second consumer reimplement a security boundary. Counts move: `ci.yml:26` nine fields → eleven, `releasing.md:35` three registries → four, `:227` three manifests → six. §6's Glama / PulseMCP / Smithery / mcp.so listing tail is **excluded** from this row on [#45](https://github.com/michal-niedzwiedzki/visimark/issues/45)'s reasoning, and §7's README correction goes out on its own docs PR. | [#169](https://github.com/michal-niedzwiedzki/visimark/issues/169) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/169#issuecomment-5786228235) |
---

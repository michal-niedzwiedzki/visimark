...
| Name | What it does | Pros | Cons | Request | Status |
|------|--------------|------|------|---------|--------|
| `NPV(rate, flows)` | Present value of a cash-flow column at per-period `rate`. Row 0 is undiscounted; row `k` is divided by `(1 + rate) ^ k`. A blank cell, an empty column, a non-column `flows`, or a `rate` of -1 or below is `TYPE`. A zero rate equals the sum of the column. The binding declares a width, including at rate zero. | The brake-press note can name the present value without a period column that drifts off by one. The rate and the row order are already in the document. Declared precision is the existing rule for division. | First reduce with a scalar argument, so the shape check has to name which parameter is the column. Excel's `NPV` discounts the first row; this one does not. A written result still declares a width. | [#157](https://github.com/michal-niedzwiedzki/visimark/issues/157) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/157#issuecomment-5782258164) |
...
| A `markdownlint` custom rule wrapping `visimark check` | New package `markdownlint-rule-visimark` exposing seventeen custom rules — one per `FindingCode` — that reconstruct the source from `params.lines` and call the existing public `analyze()`, reporting findings through `onError`, so any `markdownlint`/`markdownlint-cli2` pipeline gets visimark findings alongside its other rules via one `customRules` entry. Ships a `recommended` config fragment, consumed through `markdownlint`'s own `extends`, that switches the advisory tag off so an extending run agrees with `check`'s exit code. | Reaches an audience `markdownlint` already serves — a different toolchain population than [#152](https://github.com/michal-niedzwiedzki/visimark/issues/152)'s, not a subset of it. Needs **no new engine surface at all**: `analyze()`, `lineOf()` and `describeFinding()` are already public. Per-code rule names are the same identifiers #152 ships as `remark`'s `ruleId`, so the two integrations name the same things, and `tags` supply per-kind, per-tag and whole-package config switches with no bespoke option invented. A one-entry source-keyed cache keeps `analyze()` to one call per document despite the seventeen rules. | `markdownlint`'s rule API offers no tree, so every run pays a full re-parse with no cheaper alternative — unlike #152, which has a deferred option (b). It has no non-fatal tier either, so the advisory split lives in configuration rather than in severity, and a consumer who does not extend `recommended` sees `WARN`/`NOTE` fail a run that `check` passes. The reported count differs from `check`'s footer — folded prose anchors and suppressed advice — though the exit codes agree in every case. | [#153](https://github.com/michal-niedzwiedzki/visimark/issues/153) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/153#issuecomment-5782250622) |
---

...
| A `remark`/`unified` plugin wrapping `visimark check` | New package `remark-lint-visimark` (`packages/remark-visimark`) exposing a `unified` transformer that calls the existing public `analyze()` and reports findings as `VFile` messages, so any `remark`/`remark-lint` pipeline (Docusaurus, Astro, and the wider `unified` ecosystem) gets visimark findings inline with its other lint rules via one config line. | Reaches an audience that will never invoke the CLI directly or read the landing page — the cheapest kind of distribution this project has, per roadmap section 2's own framing. Reuses `analyze()`, `lineOf()`, `isProblem()` and `ERROR_CODES`, all already public and already tested; only one small new export (`describeFinding`) needed, no new parse-entry-point surface for the recommended (a) route. | Pays a full re-parse per document per host build (chosen deliberately over new engine surface). Version-pinned against the engine, joining `ci.yml`'s version-agreement check as a sixth/seventh file. Needs a new npm publish leg in `release.yml`, since unlike `visimark-lsp` this package is published externally. | [#152](https://github.com/michal-niedzwiedzki/visimark/issues/152) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/152#issuecomment-5781287493) |
---

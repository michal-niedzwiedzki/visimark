...
| Sheet-id / anchor-comment grammar hardening | Tightens `parseSheetId` to the identifier grammar `[A-Za-z_][A-Za-z0-9_]*` (matching `ANCHOR_RE` and the expression lexer), reporting a `SHEET` error naming the offending characters on mismatch; widens `ANCHOR` to cover any HTML comment matching the loose prefix `<!--vmark=…` that fails to fully parse, instead of silently treating it as inert HTML. | Closes a silent-failure hole where a `#hyphenated-id` sheet is legal in the fence but unanchorable and unreferenceable, so a wrong anchored value passes `check` clean and a cross-sheet reference lexes as subtraction with a useless did-you-mean. Converts a class of silent wrong answers into loud, fixable errors at declaration time — directly upholds constraint 3. Small cost, no new finding code, no example document changes; the loose-prefix rule for `ANCHOR` leaves the `<!--vmark:no-formulas-->` marker and unrelated comments untouched. | Breaking for any existing document with a non-identifier sheet id (hyphens, slashes, dots, Unicode) — it now fails `check` where it previously passed silently wrong, with no auto-fix; accepted without a changelog note since the project is pre-1.0. Loosening the anchor/lexer grammars instead is not viable — a hyphen in an expression is unavoidably minus. Does not extend to `assert`/`chart` malformed syntax, which already fails loudly today with no analogous silent-inert fallback. | [#38](https://github.com/michal-niedzwiedzki/visimark/issues/38) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/38#issuecomment-5588230949) |

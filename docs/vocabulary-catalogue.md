...
| Bun/Node parity as an always-on rule | A standing agent rule, `.agents/rules/runtime-parity.md`, linked from the rules row in `AGENTS.md`. Bun and Node are both first-class for anything this repo publishes: in-repo development stays Bun-first, Node stays a CI-tested target, a published `bin` entry points at an `sh` launcher (the pattern in `packages/visimark/bin/visimark`) rather than a `#!/usr/bin/env node` file, and a documented invocation of a published command is written in both `npx` and `bunx` form. `CONTRIBUTING.md`'s "must run under Node" paragraph cites the rule instead of standing as the only statement of half of it. No command, exit code, stream, or `--json` shape changes, and no existing CI job changes unless a later fork adds a shebang guard. | The policy is already enforced and already scattered: the #29 spec states the Bun direction, `CONTRIBUTING.md` and `acceptance-node` state the Node direction, and the launcher comment states the mechanism. None of those is loaded when an agent adds a package. #169 is the first new published `bin` since the launcher, and a stock MCP template ships the shebang that made `bun add -g visimark` fail. The file is one paragraph, reversible by deletion, and needs no release. | An always-on rule spends context on every turn, so it only earns the slot at the length drafted. The dual-form sentence is already false of the README's CI one-liner (`npx visimark check **/*.md`) and of `docs/ci.md`, which the smallest version does not rewrite. A CI grep that rejects every `node` shebang under a `bin/` directory would fail `packages/visimark/bin/visimark.js`, which #29 kept on purpose and which `acceptance-node` executes with `node`. This does not reopen the launcher ([#29](https://github.com/michal-niedzwiedzki/visimark/issues/29#issuecomment-5572442105)); it generalises the expectation that spec applied to one file. | [#171](https://github.com/michal-niedzwiedzki/visimark/issues/171) |  |
---

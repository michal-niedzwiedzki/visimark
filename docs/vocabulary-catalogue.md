...
| `simulate` command | New subcommand `visimark simulate FILE`. Every sheet with a `report` statement is asked, in document order, through one shared `eval` process, and each of its reports is printed in line order. Exit `0` when every sheet that could start has printed, including reports whose assertions do not hold. Exit `1` when the document has no `report` statement. stdout is the reports. stderr is a per-sheet fault. No `--json`. `--progress` is proposed and not specified. | One command runs every simulation the document declares. Exit `0` matches `eval`: a failed assertion is a reading, not a failed command. No built-in cap, so a large sweep is not refused by a made-up limit. | Depends on #258 for what a question is and what a report prints. Exit `1` means "nothing to run", which is not what exit `1` means for `check`. `--json` is absent, so a machine cannot consume this command the way it consumes the others. | [#259](https://github.com/michal-niedzwiedzki/visimark/issues/259) | |
---

...
| Declared local data imports with integrity stamps | A sheet may take its table from a declared local file instead of an inline GFM table: `vmark #benchmark from benchmark.csv at 4ac91fe`. `at` locks the import to a Git commit reference or a full `sha256:` digest; `fmt` writes and updates the stamp, `check` verifies and never updates it, and an unstamped import fails `check`. The imported file is input-only. Requires amending constraint 4 to admit "the contents of explicitly declared local input files", and a CSV parsing and type-conversion specification. | Large machine-generated datasets — benchmarks, measurements, exported query results — stop being unreadable 50,000-row Markdown tables while the computation and its interpretation stay in the document. The dependency and its exact identity sit adjacent in the text, so a reviewer sees both, and a changed input fails `check` until a human accepts it with `fmt`. Diffability improves rather than degrades: a one-row data change is one CSV line plus one stamp span. [§18](visimark-design.md#18-generated-artifacts) already established a tool-written local file with a hard path gate this can reuse, and no plugin point opens — data is imported, not code. | Amends the most load-bearing of the four constraints, and contests [§2](visimark-design.md#2-constraints-that-shaped-the-design)'s standing "values that genuinely come from elsewhere belong in an input column". An imported sheet has no cells, so what a column rule means there is unanswered and imported sheets are probably read-only — a language restriction, not a detail. The stamp is written into the fence info line, which [§9](visimark-design.md#9-write-back) lists as human territory, so a fourth owned category must be named. The Git stamp reintroduces ambient dependencies the digest avoids — `.git`, object reachability, a `git` binary — and cannot name a commit for uncommitted contents, forcing two commits per data change. CSV type conversion reopens thousands separators, non-ISO dates and blank cells. Cost **L–XL**, and 600,000 cells falsifies [§8](visimark-design.md#8-evaluation)'s single-digit-millisecond reparse claim. | [#66](https://github.com/michal-niedzwiedzki/visimark/issues/66) |  |

...
| Extension changelog entry for every release | A written rule in [`releasing.md`](releasing.md) for what an extension changelog entry holds, and one `ci.yml` step, running `scripts/check-changelog-entries.ts`, that fails when the root `CHANGELOG.md` or `editors/vscode/CHANGELOG.md` has no dated `## X.Y.Z - YYYY-MM-DD` heading for its own manifest's version. No CLI option, exit code, stream or `--json` shape moves; the only machine affected is `ci.yml`, which gains one failing condition. | Closes the gap v0.1.6 exposed: both files are release inputs (the GitHub Release body and the Marketplace page) and nothing opened either. Follows the version-agreement step's precedent — an unenforced checklist step gets skipped on the release where it matters. Passes on today's tree and on every PR that does not bump a version, so nothing open breaks; undone by deleting the step, with no release. | Enforces presence, not accuracy: "No editor-visible changes" when the extension did change still passes, so deciding what the language server surfaces stays manual. The heading match must be anchored (`0.1.6` must not match `0.1.60`) and does not validate the date as a calendar date. The two files' dates are not compared. Documentation debt: `releasing.md` (step 4, "Preparing the changelog", "Rules that bite") and `CONTRIBUTING.md`'s "Every version-carrying file must agree" paragraph. Cost is **S**. | [#131](https://github.com/michal-niedzwiedzki/visimark/issues/131) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/131#issuecomment-5760479248) |
---

...
| Scenario parameters (`param`, `eval --scenario`) | A `param NAME precision N = default LITERAL` statement declares a numeric scalar that may be varied from outside; every command but `eval --scenario` treats it as its default. `eval --scenario FILE\|-` evaluates the document under a strict flat JSON object of param values (unknown or non-param keys, and values wider than the declared width, are errors), quotes the scenario separately in its output, runs assertions under it, and writes nothing. `--scenario` on any other command is exit `2`. `param` and `default` become reserved words. | What-if questions stop costing an edit, a `fmt` and a revert, and stop leaving a diff nobody meant to commit. The defaults are the document: `check`, `fmt` and stored values never see a scenario, so two checkouts still agree. Strict keys and reject-not-round widths keep constraint 3. A failing assertion under a pessimistic scenario is visible in the exit code. `explain` lists the knobs an agent may vary. No new shape, no new owned category. | Needs a one-sentence amendment to [§2](visimark-design.md#2-constraints-that-shaped-the-design) constraint 4, because a scenario is an input the document does not declare. Reserving `default` can break outside documents on upgrade. A JSON number against a percent default (`12.5` for `12.5%`) is silently 1250% unless refused. Exact JSON numbers need a lossless reader. The first exception to "unrecognised options are ignored". A `param` default may not be narrowed by its declared precision, unlike every other binding ([§7](visimark-design.md#7-numeric-semantics)). Cost **M**. | [#119](https://github.com/michal-niedzwiedzki/visimark/issues/119) | |

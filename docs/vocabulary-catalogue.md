...
| `Σ` / `∑` alias for `SUM` | Adds two Unicode summation glyphs — `Σ` (U+03A3) and `∑` (U+2211) — as lexical aliases for the `SUM` reducer, resolved to `SUM` at the lexer, before the parser, arity check, evaluator, `infer`, or did-you-mean ever see them. No new semantics, no new error code, no entry in `FUNCTIONS` — evaluates, type-checks, and infers identically to `SUM(col)`. | Zero evaluation-model cost: not a second identity in the language, so it never enters a did-you-mean suggestion and cannot collide with a bound name (the identifier grammar is ASCII-only, so neither glyph was ever a legal name). A recognisable nod to conventional summation notation for a math/engineering-literate audience — an outreach/personality detail, not an expressiveness gap; `fmt` never rewrites one spelling to the other, so the author's choice persists untouched. Documented as a one-line footnote, not a headline feature — `SUM` stays canonical everywhere else, including every `infer` proposal. | The pre-review read this against the project's twice-rejected redundant-operator-spelling precedent (`%` infix, `//` division); deciding APPROVED distinguishes it as a zero-cost lexer substitution with no second identity, rather than a competing computational spelling. The explicit non-goal — no `√`, `∏`, `∫`, or any other symbol-alias system — is written into the spec rather than left to review norms alone. | [#43](https://github.com/michal-niedzwiedzki/visimark/issues/43) | [APPROVED](https://github.com/michal-niedzwiedzki/visimark/issues/43#issuecomment-5588631404) |

...
| Sheet-id / anchor-comment grammar hardening | Tightens `parseSheetId` to the identifier grammar `[A-Za-z_][A-Za-z0-9_]*` (matching `ANCHOR_RE` and the expression lexer), reporting a `SHEET` error naming the offending characters on mismatch; widens `ANCHOR` to cover any `<!--vmark=…-->` comment that fails to parse, instead of silently treating it as inert HTML. | Closes a silent-failure hole where a `#hyphenated-id` sheet is legal in the fence but unanchorable and unreferenceable, so a wrong anchored value passes `check` clean and a cross-sheet reference lexes as subtraction with a useless did-you-mean. Converts a class of silent wrong answers into loud, fixable errors at declaration time — directly upholds constraint 3. Small cost; the three example documents are unaffected. | Breaking for any existing document with a non-identifier sheet id (hyphens, slashes, dots, Unicode) — it now fails `check` where it previously passed silently wrong, with no auto-fix. The `ANCHOR` widening needs a precise grammar to avoid false positives on unrelated HTML comments and to avoid re-opening for every other malformed construct. Loosening the anchor/lexer grammars instead is not viable — a hyphen in an expression is unavoidably minus. | [#38](https://github.com/michal-niedzwiedzki/visimark/issues/38) |  |

...
| A `markdownlint` custom rule wrapping `visimark check` | New package exposing a `markdownlint` custom rule that reconstructs source from `params.lines` and calls the existing public `analyze()`, reporting findings via `onError`, so any `markdownlint`/`markdownlint-cli2` pipeline gets visimark findings alongside its other rules via one `customRules` config entry. | Reaches an audience `markdownlint` already serves — a different toolchain population than the `remark` plugin's, not a subset of it. Reuses `analyze()` and `lineOf()`, no new engine surface. | `markdownlint`'s rule API offers no tree, so every run pays a full re-parse with no cheaper alternative. Severity mapping and the exact rule id are undecided. | [#153](https://github.com/michal-niedzwiedzki/visimark/issues/153) |  |
---

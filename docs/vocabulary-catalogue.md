...
| An MCP server exposing the engine to agents | A new published package, `packages/visimark-mcp`, serving all six commands over stdio as tools, plus resources (the skill, `cli-reference.md`, `function-reference.md`, the worked invoice and its drift twin) and two prompts (take over an existing document; author a new one). Read tools take **either** `{ path }` **or** `{ content }`, so an agent can check a table it is drafting in context without writing a temp file. Writes are plan-by-default (`planFmt` / `planInfer`), applied only by a separate call, only with an operator-level `--allow-write`, and only inside host-declared roots. Imports the library as `remark-visimark` does — never shells out. Adds a publish leg to `release.yml` in the existing ask/publish-or-skip/gate shape, and a `registry.modelcontextprotocol.io` entry. | The contract work is largely done and was verified against the tree: `--json` is on all six commands (`cli-reference.md:63-68`), the three exit-code classes are live (`check` on the drift invoice exits `1` with 26 problems; a bad option exits `2`), `isProblem()` is exported, and `index.ts` already exposes the content-in API the playground and `remark-visimark` run on. Additive and opt-in — no existing CI job, script or `action.yml` behaves differently. Reaches an audience the in-repo skill cannot: an agent working in someone else's repository, which is the set this project needs to grow. The write rules are the right three and cost almost nothing, since the planners already exist. | Commits the project to a fourth published package in version lockstep, a public tool surface with no deprecation channel comparable to a CLI flag's, and a write gate that must be reviewed as a security boundary. §4's premise that the engine does no filesystem writing is wrong: `artifact/write.ts` holds the marker-and-descriptor gate and is **not** exported from `index.ts`, so the artifact leg of `visimark_apply` needs either a new public export or a reimplemented security boundary. Counts move that were not listed: `ci.yml:26,45` asserts nine version-carrying files, `releasing.md:35,227` says three registries. §6's Glama / PulseMCP / Smithery / mcp.so tail has no constraint to be judged against and is excluded from this row on [#45](https://github.com/michal-niedzwiedzki/visimark/issues/45)'s reasoning. `content` mode cannot resolve an `import` or a `chart` target, which the issue presents as interchangeable with `path`. | [#169](https://github.com/michal-niedzwiedzki/visimark/issues/169) | `NEW` |
---

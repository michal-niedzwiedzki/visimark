...
| `IMPORT` | problem | A declared local import (`from <path>`) cannot be resolved — no stamp yet, a missing file, a malformed stamp, a path outside the document's directory, malformed CSV, or a binding that shadows an imported (read-only) column. A stamp that no longer matches the file is `STALE` instead. | `visimark fmt` adds a missing stamp; everything else by hand |
